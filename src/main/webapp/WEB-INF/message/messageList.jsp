	...
	<div class="container">
		<div class="row text-center">
			...
			<div class="col-sm-10">
				...
				<div class="text left">
				받은 쪽지함 | <a href="sMessageList.cp">보낸 쪽지함</a>
				</div>
				...
				<div class="row">
					<div class="col"><c:if test="${sw==false}">현재 도착한 쪽지가 없습니다</c:if></div>
				</div>
				<div class="row text-center">
					<div class="col">
						<ul class="pagination text-center justify-content-center border-secondary">	
							<c:if test="${nowPage>1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/messageList.cp?pageSize=${pageSize}&nowPage=1&category=${category}">첫페이지</a></li></c:if>
							<c:if test="${curBlock>0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/messageList.cp?pageSize=${pageSize}&nowPage=${(curBlock-1)*blockSize+1}&category=${category}">이전블록</a></li></c:if>
							<c:forEach var="i" begin="${curBlock*blockSize+1}" end="${curBlock*blockSize + blockSize}" varStatus="st">
								...
								<c:if test="${i<=totalPage && i!= nowPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/messageList.cp?pageSize=${pageSize}&nowPage=${i}&category=${category}">${i}</a></li></c:if>
							</c:forEach>
							<c:if test="${curBlock<lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/messageList.cp?pageSize=${pageSize}&nowPage=${(curBlock+1)*blockSize+1}&category=${category}">다음블록</a></li></c:if>
							<c:if test="${nowPage<totalPage}"><li class="page-item"><a class="page-link  text-secondary" href="${ctp}/messageList.cp?pageSize=${pageSize}&nowPage=${totalPage}&category=${category}">마지막페이지</a></li></c:if>
						</ul>
					</div>
				</div>
			</div><!-- 쪽지 목록 전제부분 마지막  -->
